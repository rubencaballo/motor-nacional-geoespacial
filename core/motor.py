#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Orquestador principal: conecta todas las piezas y expone la CLI.
Import explicito de cada pieza -- esto es lo que faltaba en la version
rota anterior (motor.py no importaba nada de los demas modulos)."""
import argparse
import json
import os
import random
...
from config import (
    DB_PATH, CHECKPOINT_PATH, EXPORT_DIR, VERSION, ALTITUD_AGUACATE, log,
)
from modelos.evidencia import EvidenciaNICFI
from modelos.predio import PredioNacional, set_adaptador_ml
from core.db import init_db_nacional, checkpoint_load, checkpoint_save
from core.geo import eliminar_traslapes_jerarquico
from core.gee_backend import _medir_estado_gee_real, _clasificar_confianza_hansen
from ml.modelo import init_ml_global
...
def main():
    ...
    init_db_nacional()
    _adaptador = init_ml_global(modelo_path=args.con_ml if args.con_ml else None)
    set_adaptador_ml(_adaptador)  # conecta el ML a PredioNacional (antes se perdia por import faltante)
